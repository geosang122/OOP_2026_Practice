...
::    5. 환경 검증 테스트 실행
::
::  사용법: 이 파일을 더블클릭하거나 관리자 권한 CMD에서 실행
:: ============================================================
...
:: 사용자 PATH에 영구 등록 (재시작해도 conda 사용 가능하도록)
:: reg query로 현재 사용자 PATH 확인 후 이미 등록되어 있으면 건너뜀
...
echo !CURRENT_USER_PATH! | findstr /i /c:"miniconda3\condabin" >nul 2>&1
if %ERRORLEVEL% neq 0 (
    if "!CURRENT_USER_PATH!"=="" (
        set "NEW_PATH=%CONDA_PATHS%"
    ) else (
        set "NEW_PATH=%CONDA_PATHS%;!CURRENT_USER_PATH!"
    )
...
for /f "tokens=*" %%V in ('call conda --version 2^>^&1') do echo       - %%V
...
call conda create -n %ENV_NAME% python=%PYTHON_VER% -y -q
if %ERRORLEVEL% neq 0 (
...
call conda install -n %ENV_NAME% beautifulsoup4 pytest pillow git ipykernel -y -q
if %ERRORLEVEL% neq 0 (
...
echo       - tox를 설치합니다...
call conda run -n %ENV_NAME% pip install tox -q
if %ERRORLEVEL% neq 0 (
    echo [경고] tox 설치에 실패했습니다. 나중에 수동으로 설치하세요.
)

echo       - Jupyter 커널을 등록합니다...
call conda run -n %ENV_NAME% python -m ipykernel install --user --name %ENV_NAME% --display-name "Python 3 (OOP)" >nul 2>&1
...
if exist "%WORK_DIR%\.git" (
    echo       - 저장소가 이미 존재합니다. pull로 최신 상태로 업데이트합니다...
    pushd "%WORK_DIR%"
...
pushd "%WORK_DIR%"
call conda run -n %ENV_NAME% python tests/test_setup.py
